...
2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?
  Simple Attribute which can not be NULL.

3. How many foreign keys does the "payments" table have? How can you tell?
  The payment table has two foreign keys, customer_id and staff_id. The red colored filled diamonds indicate both are foregin keys and can not be NULL.
