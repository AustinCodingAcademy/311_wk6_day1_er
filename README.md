...
1. What is the relationship between the "actor" and "film_actor" tables?  
   They Share actor_id as a primary key.

2. What does the blue diamond next to the "last_update" column on the "inventory" table represent?
   The blue diamond indicates that last_update is not null

3. How many foreign keys does the "payments" table have? How can you tell?
   There are three, only three are highlighted. customer_id, staff_id, and rental_id. All three of them are primary keys imported from three different tables, thus they are foreign keys in the payments tbale
